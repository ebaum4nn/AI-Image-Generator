...
def generate_image(prompt: str,
    ...
    Args:
        ...
        lora_scale: Scale for LoRA application
    ...
    try:
        ...
        lora_scale = max(0.0, min(lora_scale, 2.0))

        logger.info(f"Parameters: height={height}, width={width}, steps={num_inference_steps}, guidance={guidance_scale}, lora={lora_path}, lora_scale={lora_scale}")
        ...
        if lora_path:
            try:
                ...
                logger.info("LoRA loaded and fused successfully")
            except Exception as lora_error:
                logger.warning(f"Failed to load LoRA {lora_path}: {lora_error}")
        ...
        if lora_path:
            ...
            except Exception as unfuse_error:
                logger.warning(f"Failed to unfuse LoRA: {unfuse_error}")
...
def handler(job):
    ...
    try:
        ...
        if lora_scale < 0.0 or lora_scale > 2.0:
            return {
                "error": "lora_scale must be between 0.0 and 2.0",
                "success": False
            }
